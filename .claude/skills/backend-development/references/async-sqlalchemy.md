...
class User(Base):
    ...
    version: Mapped[int] =  mapped_column(int, default=0)
    # Relationships
...
async def lifespan(app: FastAPI):
    ...
    await redis.init()
    await client.init()
    # others external singleton connection
    yield
    # Shutdown
    await engine.dispose()
    await redis.aclose()
    # others external connection close
    
app = FastAPI(lifespan=lifespan)
```

## Apply optimistick locking if table have version column:
- ❌ BAD: Do not use update statement for optimistic locking table: 
     await db.execute(
        update(User)
        .where(User.id == user_id)
