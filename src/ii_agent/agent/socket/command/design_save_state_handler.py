"""Handler for saving persisted design mode state."""
...
from ii_agent.projects.design.schemas import DesignStateRequest
...
class DesignSaveStateHandler(CommandHandler):
    """Socket handler for POST /projects/design/state."""
    ...
    def get_command_type(self) -> UserCommandType:
        return UserCommandType.DESIGN_SAVE_STATE
    ...
    async def handle(self, content: Dict[str, Any], session_info: SessionInfo) -> None:
        ...
        try:
            request = DesignStateRequest(**content)
        except Exception as exc:
            await self._send_response(
                session_info.id,
                request_id=request_id,
                success=False,
                error=f"Invalid design state save request: {exc}",
            )
            return

        try:
            async with get_db_session_local() as db:
                result = await self.container.project_design_service.save_design_state(
                    db,
                    request=request,
            ...
            logger.exception("[DesignSaveStateHandler] Failed to save design state")
            await self._send_response(
                session_info.id,
                request_id=request_id,
                success=False,
                error=f"Failed to save design state: {exc}",
            )
