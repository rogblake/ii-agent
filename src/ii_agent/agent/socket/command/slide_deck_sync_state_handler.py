"""Handler for syncing persisted slide deck design mode changes."""
...
class SlideDeckSyncStateHandler(CommandHandler):
    """Socket handler for slide deck sync (replaces POST /slides/design/slide-deck-sync-state)."""
    ...
    def get_command_type(self) -> UserCommandType:
        return UserCommandType.SLIDE_DECK_SYNC_STATE

    async def handle(self, content: Dict[str, Any], session_info: SessionInfo) -> None:
        session_id = session_info.id

        try:
            request = SlideDeckSyncStateRequest(**content)
        except Exception as exc:
            await self._send_error_event(
                session_id,
                message=f"Invalid slide deck sync-state request: {exc}",
        ...
        try:
            async with get_db_session_local() as db:
                result = await self.container.slide_design_service.sync_persisted_slide_deck_changes(
                    db,
                    request=request,
            ...
            logger.exception("[SlideDeckSyncStateHandler] Failed to sync slide deck changes")
            await self._send_error_event(
                session_id,
                message=f"Slide deck sync-state failed: {exc}",
                error_type="slide_deck_sync_state_error",
            )
