...
class AgentToolManager:
    ...
    def resolve_tools(
        ...
        for tool_name in required_tool_names:
            new_tool = AgentToolManager.convert_tool(tool_name)
            if new_tool:
                all_tools.append(new_tool)

        common_tools = AgentToolManager._get_common_tools()
    ...
    @staticmethod
    def _get_common_tools() -> List[BaseAgentTool]:
        _tools = []
    ...
    @staticmethod
    def convert_tool(tool_name: str) -> BaseAgentTool | None:
        tool_class = TOOL_CLASS_MAP.get(tool_name)
        if not tool_class:
            return None
        requires_confirmation = TOOL_CONFIRM_MAP.get(tool_name, False)
