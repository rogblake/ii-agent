...
try:
    from ii_agent.agents.models.google.gemini_schema import create_function_declaration
...
def format_function_definitions(tools: Optional[List[Any]]) -> Tool:
...
def format_image_for_message(image: Image) -> Optional[Dict[str, Any]]:
    """Convert an Image object into the bytes payload Gemini expects."""
...
class Gemini(Model):
    ...
    def _append_file_search_tool(self, builtin_tools: List[Tool]) -> None:
        ...
        logger.debug("Gemini File Search enabled.")
    ...
    def get_request_params(
        ...
        # thinking_level (Gemini 3) and thinking_budget (legacy Gemini 2.5) are mutually exclusive.
        thinking_config_params: Dict[str, Any] = {}
        if self.include_thoughts is not None:
            thinking_config_params["include_thoughts"] = self.include_thoughts
        if self.thinking_level is not None:
            # thinking_level takes precedence; do not mix with thinking_budget
        ...
        if self.vertexai_search:
            logger.info("Vertex AI Search enabled.")
            if not self.vertexai_search_datastore:
                logger.error(
                    "vertexai_search_datastore must be provided when vertexai_search is enabled."
                )
                raise ValueError(
                    "vertexai_search_datastore must be provided when vertexai_search is enabled."
                )
            builtin_tools.append(
                Tool(
                    retrieval=Retrieval(
                        vertex_ai_search=VertexAISearch(datastore=self.vertexai_search_datastore)
                    )
                )
            )
        ...
        if builtin_tools:
            if tools:
                logger.info("Built-in tools enabled. External tools will be disabled.")
        ...
        if tool_choice is not None:
            if isinstance(tool_choice, str) and tool_choice.lower() == "auto":
                config["tool_config"] = {
                    "function_calling_config": {"mode": FunctionCallingConfigMode.AUTO}
                }
            elif isinstance(tool_choice, str) and tool_choice.lower() == "none":
                config["tool_config"] = {
                    "function_calling_config": {"mode": FunctionCallingConfigMode.NONE}
                }
            elif isinstance(tool_choice, str) and tool_choice.lower() == "validated":
                config["tool_config"] = {
                    "function_calling_config": {"mode": FunctionCallingConfigMode.VALIDATED}
                }
            elif isinstance(tool_choice, str) and tool_choice.lower() == "any":
                config["tool_config"] = {
                    "function_calling_config": {"mode": FunctionCallingConfigMode.ANY}
                }
            else:
    ...
    async def ainvoke(
        ...
        try:
            ...
            logger.error("Error from Gemini API", exc_info=True)
            ...
            logger.error("httpcore read error while calling Gemini API", exc_info=True)
            raise ModelProviderError(
                message=f"Network read error: {str(e)}",
                model_name=self.name,
                model_id=self.id,
            ) from e
        except (httpx.ReadError, httpx.TimeoutException, httpx.StreamError) as e:
            logger.error("httpx error while calling Gemini API", exc_info=True)
            raise ModelProviderError(
            ...
            logger.error("Unknown error from Gemini API", exc_info=True)
    ...
    async def ainvoke_stream(
        ...
        try:
            ...
            async for chunk in async_stream:
                ...
                # Handle reasoning content (thought: true)
                ...
                # Handle regular content (thought: null/false)
                ...
                # Handle images/audio responses
                elif model_response.images or model_response.audio:
                    yield model_response
            ...
            logger.error("httpcore read error while streaming Gemini API", exc_info=True)
            raise ModelProviderError(
            ...
            logger.error("httpx error while streaming Gemini API", exc_info=True)
            raise ModelProviderError(
        ...
        for message in messages:
                ...
                if content is not None:
                    ...
                    if message.provider_data and "thought_signature" in message.provider_data:
                        part.thought_signature = base64.b64decode(
                            message.provider_data["thought_signature"]
                        )
                ...
                for idx, tool_call in enumerate(message.tool_calls):
                    ...
                    message_parts.append(
                        Part.from_function_response(
                            name=tool_call["tool_name"], response={"result": tc_content}
                        )
                    )
            ...
            if role == "user" and message.tool_calls is None:
                ...
                if message.files is not None:
                    ...
                    #     if not file.filepath:  # Only process files without filepath (content-based)
    ...
    def _format_audio_for_message(self, audio: Audio) -> Optional[Union[Part, GeminiFile]]:
        ...
        # Case 3: Audio is a local file path
            ...
            audio_path = (
                audio.filepath if isinstance(audio.filepath, Path) else Path(audio.filepath)
            )
            ...
            if (
                existing_audio_upload
                and existing_audio_upload.state
                and existing_audio_upload.state.name == "SUCCESS"
            ):
                ...
                if audio_file.state and audio_file.state.name == "FAILED":
                    logger.error(f"Audio file processing failed: {audio_file.state.name}")
                    return None
    ...
    def _format_video_for_message(self, video: Video) -> Optional[Part]:
        ...
        # Case 2: Video is stored locally
            ...
            video_path = (
                video.filepath if isinstance(video.filepath, Path) else Path(video.filepath)
            )
            ...
            if (
                existing_video_upload
                and existing_video_upload.state
                and existing_video_upload.state.name == "SUCCESS"
            ):
                ...
                if video_file.state and video_file.state.name == "FAILED":
                    logger.error(f"Video file processing failed: {video_file.state.name}")
                    return None
            ...
            if file_path.exists() and file_path.is_file():
                if file_path.stat().st_size < 20 * 1024 * 1024:  # 20MB in bytes
                    ...
                    try:
                        ...
                        logger.warning(f"Error getting file {clean_file_name}: {e}")
    ...
    def _parse_provider_response(
        self, response: GenerateContentResponse, **kwargs
    ) -> ModelResponse:
        """
                ...
                for url_meta in url_metadata_list:
                    ...
                    if retrieved_url and status == "URL_RETRIEVAL_STATUS_SUCCESS":
                        ...
                        if retrieved_url not in existing_urls:
                            citations_urls.append(
                                UrlCitation(url=retrieved_url, title=retrieved_url)
                            )
    ...
    def _parse_provider_response_delta(
        self,
        response_delta: GenerateContentResponse,
    ) -> ModelResponse:
        """
        Parse a single streaming chunk from Gemini into a ModelResponse.

        This method just extracts content - state management and event emission
        are handled by ainvoke_stream.

        Args:
            response_delta: Raw streaming chunk from Gemini

        Returns:
            ModelResponse: Parsed response data (content, reasoning_content, tool_calls, etc.)
        """
        model_response = ModelResponse()
        ...
        if response_delta.candidates and len(response_delta.candidates) > 0:
            ...
            if response_message.parts is not None:
                for part in response_message.parts:
                    ...
                    if hasattr(part, "text") and part.text is not None:
                        ...
                        # Check if this is a thought/reasoning (thought: true)
                        if hasattr(part, "thought") and part.thought:
                            model_response.reasoning_content = text_content
                        else:
                            model_response.content = text_content
                    ...
                    # Process inline data (audio/images)
                    if hasattr(part, "inline_data") and part.inline_data is not None:
                        # Audio responses
                        if part.inline_data.mime_type and part.inline_data.mime_type.startswith(
                            "audio/"
            ...
            # Capture usage metadata (will be accumulated by ainvoke_stream)
            if self._has_usage_metadata(response_delta):
    ...
    async def async_list_file_search_stores(self, page_size: int = 100) -> List[Any]:
        ...
        try:
            stores = []
            async for store in await self.get_client().aio.file_search_stores.list(
    ...
    def get_file_search_store(self, name: str) -> Any:
        ...
        try:
            ...
            logger.debug(f"Retrieved File Search store: {name}")
            return store
        except Exception as e:
            logger.error(f"Error getting File Search store {name}: {e}")
            raise
    ...
    def delete_file_search_store(self, name: str, force: bool = False) -> None:
        ...
        try:
            ...
            logger.error(f"Error deleting File Search store {name}: {e}")
            raise
    ...
    async def async_delete_file_search_store(self, name: str, force: bool = True) -> None:
        ...
        """
        try:
            await self.get_client().aio.file_search_stores.delete(
                name=name, config={"force": force}
            )
            logger.info(f"Deleted File Search store: {name}")
        except Exception as e:
            logger.error(f"Error deleting File Search store {name}: {e}")
    ...
    def wait_for_operation(
        ...
        while not operation.done:
            ...
            logger.debug(f"Waiting for operation... ({elapsed}s elapsed)")
    ...
    def upload_to_file_search_store(
        ...
        try:
            logger.info(f"Uploading file {file_path.name} to File Search store {store_name}")
            operation = self.get_client().file_search_stores.upload_to_file_search_store(
                file=file_path,
                file_search_store_name=store_name,
                config=config or None,  # type: ignore[arg-type]
            )
            logger.info(f"Upload initiated for {file_path.name}")
            return operation
        except Exception as e:
    ...
    def import_file_to_store(
        ...
        try:
            logger.info(f"Importing file {file_name} to File Search store {store_name}")
            operation = self.get_client().file_search_stores.import_file(
                file_search_store_name=store_name,
                file_name=file_name,
                config=config or None,  # type: ignore[arg-type]
            )
            logger.info(f"Import initiated for {file_name}")
            return operation
        except Exception as e:
            logger.error(f"Error importing file to File Search store: {e}")
            raise
    ...
    def list_documents(self, store_name: str, page_size: int = 20) -> List[Any]:
        ...
        try:
            ...
            logger.error(f"Error listing documents in store {store_name}: {e}")
            raise
    ...
    def get_document(self, document_name: str) -> Any:
        ...
        try:
            ...
            logger.debug(f"Retrieved document: {document_name}")
            return doc
        except Exception as e:
            logger.error(f"Error getting document {document_name}: {e}")
            raise
