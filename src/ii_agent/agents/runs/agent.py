...
from ii_agent.agents.runs.base import BaseRunOutputEvent, MessageReferences, RunStatus
...
from ii_agent.agents.utils.media import (
...
class RunContentCompletedEvent(BaseAgentRunEvent):
    event: str = "RunContentCompleted"


@dataclass
class SandboxInitializedEvent(BaseAgentRunEvent):
    event: str = "SandboxInitialized"
...
class AgentSummaryStartedEvent(BaseAgentRunEvent):
    event: str = "SessionSummaryStarted"


@dataclass
class AgentSummaryCompletedEvent(BaseAgentRunEvent):
    event: str = "SessionSummaryCompleted"
...
class OutputModelResponseStartedEvent(BaseAgentRunEvent):
    event: str = "OutputModelResponseStarted"


@dataclass
class OutputModelResponseCompletedEvent(BaseAgentRunEvent):
    event: str = "OutputModelResponseCompleted"


@dataclass
class ModelTurnMetricsEvent(BaseAgentRunEvent):
    """Emitted after each LLM API turn with per-turn token usage.

    The ``metrics`` field carries per-turn :class:`Metrics` populated from
    ``ModelResponse.response_usage`` in the streaming path.  Billing
    subscribers react to this event for per-call credit deduction.
    """

    event: str = "ModelTurnMetrics"
    metrics: Optional[Metrics] = None
    model_id: str = ""
...
# Backward-compat alias used in events.py for type hints
...
RUN_EVENT_TYPE_REGISTRY = {
    ...
    "OutputModelResponseStarted": OutputModelResponseStartedEvent,
    "OutputModelResponseCompleted": OutputModelResponseCompletedEvent,
    "SandboxInitialized": SandboxInitializedEvent,
    "ModelTurnMetrics": ModelTurnMetricsEvent,
