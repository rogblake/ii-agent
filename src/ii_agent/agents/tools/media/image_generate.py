from ii_agent.agents.tools.sandbox import BaseSandboxTool
import httpx
...
INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "model_name": {
            "type": "string",
            "description": "Optional image model name to use for generation",
        },
        "provider": {
            "type": "string",
            "description": "Optional provider name such as gemini, vertex, openai, or fal",
        },
    },
...
class ImageGenerateTool(BaseSandboxTool):
    ...
    async def execute(
        ...
        try:
            response = await tool_client.generate_image(
