...
MAX_RESULTS = 6
FAILURE_MESSAGE = "Please try again. If the problem continues, switch to `agent-browser` skill for manual searching and let the user know that web search is temporarily unavailable."
...
class WebBatchSearchTool(BaseAgentTool):
    ...
    async def execute(
        ...
        try:
            response = await tool_client.web_batch_search(queries, max_results=MAX_RESULTS)
