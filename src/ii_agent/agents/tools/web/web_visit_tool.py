...
FAILURE_MESSAGE = "Please try again. If the problem continues, switch to `agent-browser` skill for manual visiting and let the user know that web visit is temporarily unavailable."
...
class WebVisitTool(BaseAgentTool):
    ...
    async def execute(
        ...
        try:
            response = await tool_client.web_visit(url, prompt)
        ...
        if content is None or (isinstance(content, str) and not content.strip()):
            return ToolResult(
                llm_content="The webpage content is empty or un-extractable (e.g. login, paywall, etc.). Please try again with a different URL or use `agent-browser` skill to visit manually.",
                is_error=True,
            )
