...
from ii_agent.billing.policy import (
    ...
    controlled_shortfall_budget_credits,
)
from ii_agent.billing.operations import (
    capture_billing_result,
    reserve_billing_operation,
    settle_billing_result,
)
...
class LLMBillingService:
    ...
    async def reserve_chat_llm_call(
        ...
        *,
        ...
        source_domain: str | SourceDomain = SourceDomain.CHAT_LLM,
    ) -> ReservedLLMCall | None:
        """Reserve one chat LLM call before sending it to the provider."""
        if llm_config.is_user_model():
            return None

        resolved_source_domain = self._source_domain_value(source_domain)
        ...
        if requested_output_token_cap is not None and requested_output_token_cap > 0:
            model_cap = min(model_cap, requested_output_token_cap)
        idempotency_key = scope.build_operation_key(resolved_source_domain, source_id)
        hold, output_cap = await self._reserve_llm_call(
            db,
            scope=scope,
        ...
        if hold is None:
            return None
        return self._build_reserved_llm_call(
            hold=hold,
            input_tokens_estimate=input_tokens,
    ...
    async def reserve_agent_llm_call(
        ...
        *,
        ...
        messages: list[Any],
        source_id: str,
        source_domain: str | SourceDomain = SourceDomain.AGENT_LLM,
    ) -> ReservedLLMCall | None:
    ...
    async def settle_llm_call(
        ...
        *,
        ...
        """Settle one reserved LLM call to actual provider usage."""
    ...
    async def manual_retry_settlement(
        self,
        db: AsyncSession,
        *,
        reservation_id: str,
    ) -> BillingSettlementResult:
        """Replay a failed settlement manually from captured reservation metadata."""
        return await self._reservation_service.retry_settlement_from_capture(
            db,
            reservation_id=reservation_id,
        )
    ...
    async def _settle_llm_direct(
        ...
        *,
        ...
        usage_payload = {
            "run_id": self._scope_run_id(scope),
            "provider": provider,
            "request_kind": request_kind,
        ...
        return await self._settle_direct_operation(
            db,
            scope=scope,
            reservation_id=reservation_id,
    ...
    async def record_zero_cost_tool_usage(
        ...
        *,
        ...
        tool_name: str,
        succeeded: bool,
        source_domain: str | SourceDomain = SourceDomain.AGENT_TOOL,
    ) -> int | None:
        ...
        return await self._usage_service.record_settled_usage(
            ...
            run_id=scope.run_id,
            amount=0.0,
            source_domain=self._source_domain_value(source_domain),
            billing_kind=BillingKind.TOOL_USAGE,
            tool_name=tool_name,
            cost_usd=0.0,
            app_kind=scope.app_kind,
            usage_metadata={
                "billing_context": scope.billing_context,
                "tool_name": tool_name,
                "succeeded": succeeded,
    ...
    async def _settle_direct_operation(
        self,
        db: AsyncSession,
        *,
        scope: BillingScope,
        ...
        result = self._build_direct_billing_result(
            actual_usd=actual_usd,
            actual_credits=actual_credits,
            usage_payload=usage_payload,
        )
        ...
        await db.commit()
        return await settle_billing_result(
            db,
            reservation_service=self._reservation_service,
            scope=scope,
            reservation_id=reservation_id,
    ...
    @staticmethod
    def _build_direct_billing_result(
        *,
        actual_usd: Decimal,
        actual_credits: Decimal,
        usage_payload: dict[str, Any],
    ...
    @staticmethod
    def _source_domain_value(value: str | SourceDomain) -> str:
        return value.value if isinstance(value, SourceDomain) else str(value)

    @staticmethod
    def _scope_run_id(scope: BillingScope) -> str | None:
    ...
    async def _quote_llm_call(
        ...
        *,
        ...
        input_tokens: int,
        model_cap: int,
        available_credits: float | Decimal | None = None,
        ...
        resolved_available_credits = (
            Decimal(str(available_credits))
            if available_credits is not None
            else await self._fetch_available_credits(db, user_id)
        )
        effective_budget_credits = controlled_shortfall_budget_credits(resolved_available_credits)
        available_usd = credits_to_usd(float(effective_budget_credits))
        ...
        if output_cap < _MIN_USEFUL_OUTPUT_TOKENS:
            raise InsufficientCreditsError(
                ...
                phase="reserve",
                available_credits=float(resolved_available_credits),
        ...
        return (
            BillingQuote(
                ...
                metadata={
                    ...
                    "output_token_cap": output_cap,
                    "controlled_shortfall_budget_credits": float(CONTROLLED_SHORTFALL_MAX_CREDITS),
                },
            ),
            output_cap,
        )
    ...
    async def _reserve_llm_call(
        ...
        *,
        ...
        input_tokens: int,
        model_cap: int,
    ) -> tuple[ReservationHold | None, int]:
        output_cap = model_cap

        existing_hold = await self._reservation_service.get_hold_by_idempotency_key(
            db,
            idempotency_key=request.idempotency_key,
        )
        if existing_hold is not None:
            return existing_hold, int(existing_hold.output_token_cap or model_cap)

        available_credits = await self._fetch_available_credits(db, scope.user_id)
        ...
        return hold, output_cap

    async def _fetch_available_credits(self, db: AsyncSession, user_id: str) -> Decimal:
        balance = await self._credit_service.get_balance(db, user_id)
        if balance is None:
            return Decimal("0")
        return Decimal(str(balance.credits + balance.bonus_credits))
