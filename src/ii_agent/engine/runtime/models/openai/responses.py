...
from ii_agent.engine.runtime.utils.openai import images_to_message
...
from ii_agent.engine.runtime.exceptions import ModelAuthenticationError, ModelProviderError
...
from ii_agent.engine.runtime.utils.http import get_default_async_client, get_default_sync_client
...
class OpenAIResponses(Model):
    ...
    def get_request_params(
        ...
        if "deep-research" in self.id:
            ...
            if not has_web_search:
                ...
                logger.debug(f"Added web_search_preview tool for deep research model: {self.id}")
    ...
    def _format_tool_params(
        ...
        # TODO: reserve this logics for chat
    ...
    async def ainvoke(
        ...
        try:
            ...
            logger.error(f"API connection error from OpenAI API: {exc}")
            raise ModelProviderError(
                message=str(exc), model_name=self.name, model_id=self.id
            ) from exc
            ...
            logger.error(f"Model authentication error from OpenAI API: {exc}")
            raise exc
        except httpcore.ReadError as exc:
            logger.error(f"HTTP read error while calling OpenAI API: {str(exc)}")
            raise ModelProviderError(
                message=f"Network read error: {str(exc)}",
                model_name=self.name,
                model_id=self.id,
            ) from exc
        except httpx.ReadError as exc:
            logger.error(f"HTTPX read error while calling OpenAI API: {str(exc)}")
            raise ModelProviderError(
                message=f"Network read error: {str(exc)}",
                model_name=self.name,
                model_id=self.id,
            ...
            logger.error(f"Timeout while calling OpenAI API: {str(exc)}")
            raise ModelProviderError(
            ...
            logger.error(f"Stream error while calling OpenAI API: {str(exc)}")
            raise ModelProviderError(
            ...
            logger.error(f"Error from OpenAI API: {exc}")
            raise ModelProviderError(
                message=str(exc), model_name=self.name, model_id=self.id
            ) from exc
        ...
        try:
            ...
            async for chunk in async_stream:  # type: ignore
                model_response, tool_use = self._parse_provider_response_delta(
                    chunk, assistant_message, tool_use
                )  # type: ignore
            ...
            logger.error(f"HTTP read error while streaming OpenAI API: {str(exc)}")
            raise ModelProviderError(
                message=f"Network read error during streaming: {str(exc)}",
                model_name=self.name,
                model_id=self.id,
            ) from exc
        except httpx.ReadError as exc:
            logger.error(f"HTTPX read error while streaming OpenAI API: {str(exc)}")
            raise ModelProviderError(
                message=f"Network read error during streaming: {str(exc)}",
                model_name=self.name,
                model_id=self.id,
            ...
            logger.error(f"Timeout while streaming OpenAI API: {str(exc)}")
            raise ModelProviderError(
                message=f"Request timeout during streaming: {str(exc)}",
                model_name=self.name,
                model_id=self.id,
            ) from exc
        except httpx.StreamError as exc:
            logger.error(f"Stream error while streaming OpenAI API: {str(exc)}")
            raise ModelProviderError(
                message=f"Stream error: {str(exc)}",
                model_name=self.name,
                model_id=self.id,
        ...
        for output in response.output:
            ...
            if output.type == "message":
                ...
                for content in output.content:
                    if content.type == "output_text" and content.annotations:
                        citations.raw = [
                            annotation.model_dump() for annotation in content.annotations
                        ]
            ...
            # Handle reasoning output items
                ...
                if self.store is False:
                    if model_response.provider_data is None:
                        model_response.provider_data = {}
                    model_response.provider_data["reasoning_output"] = output.model_dump(
                        exclude_none=True
                    )
    ...
    def _parse_provider_response_delta(
        ...
        # 2. Add citations
        if stream_event.type == "response.output_text.annotation.added":
            ...
            if isinstance(stream_event.annotation, dict):
                if stream_event.annotation.get("type") == "url_citation":
                    ...
                    model_response.citations.urls.append(
                        UrlCitation(
                            url=stream_event.annotation.get("url"),
                            title=stream_event.annotation.get("title"),
                        )
                    )
            else:
                if stream_event.annotation.type == "url_citation":  # type: ignore
                    ...
                    model_response.citations.urls.append(
                        UrlCitation(
                            url=stream_event.annotation.url,
                            title=stream_event.annotation.title,
                        )  # type: ignore
                    )
        if stream_event.type == "response.output_text.done":
            model_response.content = stream_event.text
        ...
        if stream_event.type == "response.reasoning_summary_part.added":
            model_response.delta_status = "reasoning_started"

        # 3.2 Add reasoning summary delta
        if stream_event.type == "response.reasoning_summary_text.delta":
            # Reasoning summary streaming when reasoning_summary is enabled
            model_response.reasoning_content = stream_event.delta
            model_response.delta_status = None
            model_response.is_delta = True

        if stream_event.type == "response.reasoning_summary_text.done":
            # Capture completed reasoning summary text
            reasoning_summary = stream_event.text
            # Signal reasoning completion
            model_response.delta_status = "reasoning_done"
            model_response.is_delta = False
            model_response.reasoning_content = reasoning_summary

        # 4.1 Add starting tool call
        if stream_event.type == "response.output_item.added":
        ...
        if stream_event.type == "response.function_call_arguments.delta":
            tool_use["function"]["arguments"] += stream_event.delta

        # 4.3 Add tool call completion data
        if stream_event.type == "response.output_item.done" and tool_use:
                ...
                for out in getattr(stream_event.response, "output", []) or []:
                    if getattr(out, "type", None) == "reasoning":
                        ...
                        if self.store is False and hasattr(out, "encrypted_content"):
                            ...
                            model_response.provider_data["reasoning_output"] = out.model_dump(
                                exclude_none=True
                            )
                        if self.reasoning_summary is not None:
    ...
    def _get_metrics(self, response_usage: ResponseUsage) -> Metrics:
        ...
        if input_tokens_details := response_usage.input_tokens_details:
            if input_tokens_details.cached_tokens is not None:
                metrics.cache_read_tokens = input_tokens_details.cached_tokens
                input_tokens = response_usage.input_tokens - input_tokens_details.cached_tokens
        if output_tokens_details := response_usage.output_tokens_details:
            if output_tokens_details.reasoning_tokens is not None:
