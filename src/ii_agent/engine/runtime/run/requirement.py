...
from ii_agent.engine.runtime.models.response import ToolExecution, UserInputField
...
class RunRequirement:
    ...
    def needs_user_input(self) -> bool:
        ...
        if self.tool_execution.answered is True:
            return False
        if self.user_input_schema and not all(
            field.value is not None for field in self.user_input_schema
        ):
            return True
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Serialize RunRequirement to dictionary for JSON storage."""
        _dict: Dict[str, Any] = {
        ...
        if self.tool_execution is not None:
            _dict["tool_execution"] = self.tool_execution.to_dict()
