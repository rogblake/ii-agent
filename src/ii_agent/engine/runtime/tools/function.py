...
from ii_agent.engine.runtime.utils.json_schema import get_py_type_for_json_type
...
from ii_agent.engine.runtime.tools.base import BaseAgentTool, UserInputField
...
class Function(BaseModel):
    ...
    def from_callable(
        ...
        from ii_agent.engine.runtime.utils.json_schema import get_json_schema
    ...
    @classmethod
    def from_tool(cls, tool: "BaseAgentTool") -> "Function":
        """Create a Function from a BaseTool instance.

        Args:
            tool: A BaseTool instance with name, description, input_schema, and execute method

        Returns:
            Function instance that wraps the BaseTool
        """
        ...
        # Create an async wrapper that calls tool.execute and returns the full ToolResult
        # Note: agent parameter must be explicit (not in **kwargs) for _build_entrypoint_args
        # signature inspection to detect it and inject function._agent
        async def tool_entrypoint(**kwargs) -> ToolResult:
            """Wrapper that executes the BaseTool and returns the full ToolResult.

            Returns the complete ToolResult which contains:
            - llm_content: Content to send to the LLM
            - user_display_content: Content to display via websocket
            ...
            try:
                ...
                logger.error(f"Error executing BaseTool {tool.name}: {e}")
                return ToolResult(
                    llm_content=f"Error: {str(e)}",
                    user_display_content=f"Error executing {tool.name}: {str(e)}",
                    is_error=True,
                )

        # Set the function name for better debugging
        tool_entrypoint.__name__ = tool.name
        ...
        if tool.requires_user_input and tool.user_input_fields and tool.input_schema:
            properties = tool.input_schema.get("properties", {})
            user_input_schema = []
            for field_name in tool.user_input_fields:
                if field_name in properties:
                    field_props = properties[field_name]
                    json_type = field_props.get("type", "string")
                    python_type = get_py_type_for_json_type(json_type)
        ...
        return cls(
            ...
            pre_hook=tool.on_tool_start,
            post_hook=tool.on_tool_end,
            entrypoint=tool_entrypoint,
            requires_sandbox=tool.requires_sandbox,
            requires_confirmation=tool.requires_confirmation,
            # Pre-execution HITL fields
            requires_user_input=tool.requires_user_input,
            user_input_fields=tool.user_input_fields,
            user_input_schema=user_input_schema,
            # Stop agent after tool execution
    ...
    def process_entrypoint(self, strict: bool = False):
        ...
        try:
            ...
            param_type_hints = {
                name: type_hints.get(name) for name in sig.parameters if name not in excluded_params
            }
            ...
            if strict:
                parameters["required"] = [
                    name for name in parameters["properties"] if name not in excluded_params
                ]
            else:
...
class FunctionCall(BaseModel):
    ...
    def get_call_str(self) -> str:
        ...
        call_str = (
            f"{self.function.name}({', '.join([f'{k}={v}' for k, v in trimmed_arguments.items()])})"
        )
    ...
    def _build_hook_args(
        self, hook: Callable, name: str, func: Callable, args: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _build_nested_execution_chain(self, entrypoint_args: Dict[str, Any]):
        ...
        for hook in self.function.tool_hooks:
            if iscoroutinefunction(hook):
                logger.warning(
                    f"Cannot use async hooks with sync function calls. Skipping hook: {hook.__name__}"
                )
            else:
        ...
        try:
            ...
            if self.function.tool_hooks is not None:
                ...
                result = execution_chain(
                    self.function.name, self.function.entrypoint, self.arguments or {}
                )
            else:
            ...
            if isgenerator(result):
                self.result = result  # Store generator directly
            else:
                self.result = result
    ...
    async def _spin_up_hook_if_requires(self):
        if self.function.requires_sandbox:
    ...
    async def _handle_pre_hook_async(self):
        ...
        await self._spin_up_hook_if_requires()
        if self.function.pre_hook is not None:
            try:
                from inspect import signature
    ...
    async def _build_nested_execution_chain_async(self, entrypoint_args: Dict[str, Any]):
        ...
        async def execute_entrypoint_async(name, func, args):
            ...
            if iscoroutinefunction(self.function.entrypoint) and not isasyncgenfunction(
