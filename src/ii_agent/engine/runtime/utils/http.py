...
def get_default_async_client() -> httpx.AsyncClient:
    ...
    if _global_async_client is None or _global_async_client.is_closed:
        _global_async_client = httpx.AsyncClient(
            ...
            http2=True,
            follow_redirects=True,
            # Set timeout to prevent server-side stream resets on long-running requests
            # None for read timeout allows streaming responses to take as long as needed
...
def set_default_sync_client(client: httpx.Client) -> None:
    ...
    Example:
        >>> import httpx
        >>> from ii_agent.v2.utils.http import set_default_sync_client
...
def set_default_async_client(client: httpx.AsyncClient) -> None:
    ...
    Example:
        >>> import httpx
        >>> from ii_agent.v2.utils.http import set_default_async_client
