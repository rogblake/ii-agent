...
def download_image(url: str, output_path: str) -> bool:
    ...
    try:
        ...
        logger.warning(f"Error downloading the image: {e}")
        return False
    except IOError as e:
        logger.warning(f"Error saving the image to '{output_path}': {e}")
        return False
...
def wait_for_media_ready(
    url: str, timeout: int = 120, interval: int = 5, verbose: bool = True
) -> bool:
    """
    ...
    if verbose:
        logger.warning(f"Timeout waiting for media. Try this URL later: {url}")
    return False
...
def reconstruct_image_from_dict(img_data):
    ...
    try:
        ...
        logger.warning(f"Failed to reconstruct image from dict: {e}")
        return None
...
def reconstruct_video_from_dict(vid_data):
    ...
    try:
        ...
        logger.warning(f"Failed to reconstruct video from dict: {e}")
        return None
...
def reconstruct_audio_from_dict(aud_data):
    ...
    try:
        ...
        logger.warning(f"Failed to reconstruct audio from dict: {e}")
        return None
...
def reconstruct_file_from_dict(file_data):
    ...
    try:
        ...
        logger.warning(f"Failed to reconstruct file from dict: {e}")
        return None
