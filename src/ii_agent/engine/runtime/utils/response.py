...
from ii_agent.engine.runtime.run.agent import RunOutput, RunOutputEvent, RunPausedEvent
...
def format_tool_calls(tool_calls: List[ToolExecution]) -> List[str]:
    ...
    for tool_call in tool_calls:
        if tool_call.tool_name is not None:
            ...
            args_str = ""
            if (
                tool_call.tool_args is not None and tool_call.tool_args
            ):  # Check if args exist and are non-empty
...
def get_paused_content(run_output: RunOutput) -> str:
    ...
    for tool in run_output.tools or []:
        ...
        if (
            ...
            and not tool.confirmed
        ):
            confirmation_required = True
        if tool.requires_user_input is not None and tool.requires_user_input is True:
            user_input_required = True
        if (
            tool.external_execution_required is not None
            and tool.external_execution_required is True
        ):
