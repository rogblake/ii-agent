...
from ii_agent.engine.v1.models.message import Citations, Message
...
from ii_agent.engine.v1.run.requirement import RunRequirement
from ii_agent.engine.v1.tools.function import (
    Function,
    FunctionCall,
...
class Model(ABC):
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                if attempt < self.retries:
                    ...
                    logger.error(f"Model provider error after {self.retries + 1} attempts: {e}")
    ...
    async def aresponse(
        ...
        run_response: Optional[Union[RunOutput, RunOutput]] = None,
        ...
        try:
            ...
            _functions = (
                {tool.name: tool for tool in tools if isinstance(tool, Function)}
            ...
            while True:
                ...
                if assistant_message.tool_calls:
                        ...
                        if isinstance(function_call_response, ModelResponse):
                            ...
                            if function_call_response.updated_session_state is not None:
                                model_response.updated_session_state = (
                                    function_call_response.updated_session_state
                                )
                            ...
                            if (
                                ...
                                model_response.tool_executions.extend(
                                    function_call_response.tool_executions
                                )
                                ...
                                if (
                                    ...
                                    current_tool_execution = function_call_response.tool_executions[
                                        -1
                                    ]
                    ...
                    if any(
                        msg.images or msg.videos or msg.audio or msg.files
                    ...
                    if any(
                        tc.external_execution_required
            ...
            if self.__class__.__name__ in ["Gemini", "GeminiInteractions"] and self.client is not None:
                try:
                    ...
                    logger.warning(
                        "Your Gemini client is outdated. For to properly handle the lifecycle of the client,"
    ...
    async def _aprocess_model_response(
        ...
        self._populate_assistant_message(
            assistant_message=assistant_message, provider_response=provider_response
        )
    ...
    def _populate_assistant_message(
        ...
        if provider_response.images is not None:
            if provider_response.images:
                assistant_message.image_output = provider_response.images[
                    -1
                ]  # Taking last (most recent) image

        # Add video to assistant message
        if provider_response.videos is not None:
            if provider_response.videos:
                assistant_message.video_output = provider_response.videos[
                    -1
                ]  # Taking last (most recent) video

        if provider_response.files is not None:
            if provider_response.files:
                assistant_message.file_output = provider_response.files[
                    -1
                ]  # Taking last (most recent) file

        if provider_response.audios is not None:
            if provider_response.audios:
                assistant_message.audio_output = provider_response.audios[
                    -1
                ]  # Taking last (most recent) audio

        # Add redacted thinking content to assistant message
        if provider_response.redacted_reasoning_content is not None:
            assistant_message.redacted_reasoning_content = (
                provider_response.redacted_reasoning_content
            )
    ...
    async def aresponse_stream(
        ...
        send_media_to_model: bool = True,
    ) -> AsyncIterator[Union[ModelResponse, RunOutputEvent, RunOutputEvent]]:
        """
        Generate an asynchronous streaming response from the model.
        """
        try:
            logger.debug(
                f"{self.get_provider()} Async Response Stream Start",
                center=True,
                symbol="-",
            )
            logger.debug(f"Model: {self.id}", center=True, symbol="-")
            ...
            logger.debug(
                f"{self.get_provider()} Async Response Stream End",
                center=True,
                symbol="-",
            )
        finally:
    ...
    def _populate_stream_data(
        ...
        """Update the accumulate stream data for each event and assistant message with the model response."""
    ...
    async def arun_function_calls(
        ...
        for i, original_result in enumerate(results):
            ...
            if async_function_call_output is not None:
                ...
                from ii_agent.engine.v1.tools.function import ToolResult as FunctionToolResult
                from ii_agent.engine.v1.tools.base import ToolResult as BaseToolResult
