...
from ii_agent.engine.v1.models.message import Citations
...
from ii_agent.engine.v1.agent_sessions.summary import SessionSummary
...
def handle_event(
    ...
    Args:
        ...
        run_response: The current run response
        events_to_skip: List of event types to skip persisting to DB
        store_events: Whether to persist events to database

    Returns:
        The same event passed in
    """
    # We only persist events that are not in events_to_skip
    events_to_skip_list = [e.value for e in events_to_skip] if events_to_skip else []

    # Skip persisting this event if it's in the skip list
    if event.event in events_to_skip_list:
        return event

    # Persist to database asynchronously (fire-and-forget)
    if store_events:
        asyncio.create_task(_persist_event_to_db(event, run_response))
...
async def _persist_event_to_db(
    ...
    """Persist event to database in background.
    ...
    """
    try:
        from ii_agent.engine.v1.db.agent import AgentEvent
