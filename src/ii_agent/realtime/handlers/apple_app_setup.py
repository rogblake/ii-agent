...
from ii_agent.realtime.schemas import AppleAppSetupContent, AppleListAppsContent

logger = logging.getLogger(__name__)


class AppleAppSetupHandler(BaseCommandHandler[AppleAppSetupContent]):
    ...
    _content_type = AppleAppSetupContent
    ...
    async def handle(
        ...
        """
        bundle_id = content.bundle_identifier.strip()
        app_name = content.app_name.strip()
        # Password can come from the request (preferred) or from stored session
        request_password = content.password.strip() if content.password else ""
        ...
        try:
            ...
            try:
                ...
                await self._send_error_event(
                    session_info.id,
                    error_code=ErrorCode.NAME_TAKEN,
                    message=str(e),
                    app_name=app_name,
                )
                ...
                await self._send_error_event(
                    session_info.id,
                    error_code=ErrorCode.BUNDLE_ID_TAKEN,
                    message=str(e),
                    bundle_id=bundle_id,
                )
            ...
            if is_auth_error:
                ...
                await self._send_error_event(
                    session_info.id,
                    error_code=ErrorCode.BUNDLE_ERROR,
                    message=f"Bundle ID error: {e}",
                )
...
class AppleListAppsHandler(BaseCommandHandler[AppleListAppsContent]):
    """Handler for listing existing apps from App Store Connect."""

    _content_type = AppleListAppsContent
