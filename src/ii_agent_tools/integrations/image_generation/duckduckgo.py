...
class DuckDuckGoImageGenerationClient(BaseImageGenerationClient):
    ...
    async def generate_from_images(
        ...
        **_: Any,
        ...
        raise ImageGenerationError("DuckDuckGo provider does not support image-to-image generation")
    ...
    def _sanitize_results(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for result in results:
            image_url = result.get("image") or result.get("thumbnail") or result.get("url") or ""
            if not image_url:
                continue
