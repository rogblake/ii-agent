...
class FakeBillingService:
    def __init__(
        ...
        *,
        ...
        tool_settle_result=None,
    ):
        self.reserve_calls = []
        self.settle_calls = []
        self.release_calls = []
        self.reserve_tool_calls = []
        self.settle_tool_calls = []
        self.release_tool_calls = []
        self.zero_cost_tool_calls = []
        self.mark_failed_calls = []
        self._reservation = reservation
        self._settle_result = settle_result
        self._tool_reservation = tool_reservation
        self._tool_settle_result = tool_settle_result
    ...
    async def settle_llm_call(self, db, **kwargs):
    ...
    async def reserve_tool_call(self, db, **kwargs):
        self.reserve_tool_calls.append(kwargs)
        return self._tool_reservation

    async def settle_tool_call(self, db, **kwargs):
        self.settle_tool_calls.append(kwargs)
        return self._tool_settle_result

    async def release_tool_call(self, db, *, reservation, reason):
        self.release_tool_calls.append(SimpleNamespace(reservation=reservation, reason=reason))
        return None

    async def record_zero_cost_tool_usage(self, db, **kwargs):
        self.zero_cost_tool_calls.append(kwargs)
        return None
    ...
    async def mark_settlement_failed(self, db, *, reservation_id, error):
        ...
        self.mark_failed_calls.append(SimpleNamespace(reservation_id=reservation_id, error=error))
...
class FailingCallerDB:
    def begin_nested(self):
        raise AssertionError("caller session should not be used for llm invocation telemetry")
...
def _patch_billing_session(monkeypatch):
    @asynccontextmanager
    async def _fake_billing_session():
        yield FakeDB()

    monkeypatch.setattr(
        "ii_agent.core.llm.execution_service.get_db_session_local",
        _fake_billing_session,
    )
...
async def test_send_once_reserves_settles_on_success():
    ...
    assert billing.reserve_calls[0]["scope"].user_id == "u1"
    assert billing.reserve_calls[0]["scope"].subject.id == "s1"
    assert billing.settle_calls[0]["scope"].subject.id == "s1"
    assert len(invocation_repo.calls) == 1
    assert invocation_repo.calls[0].finish_reason == "end_turn"


@pytest.mark.asyncio
async def test_send_once_forwards_custom_llm_source_domain():
    llm_config = LLMConfig(model="gpt-4o", api_type=APITypes.OPENAI, config_type="system")
    ...
    billing_context = LLMBillingContext(
        ...
        llm_config=llm_config,
        model_id=llm_config.model,
        llm_source_domain="storybook_llm",
    )
    ...
    await service.send_once(
        ...
        usage_key="test_storybook_llm",
    )

    assert billing.reserve_calls[0]["source_domain"] == "storybook_llm"
...
async def test_send_once_uses_reservation_provider_options():
    ...
    llm_config = LLMConfig(model="gpt-4o", api_type=APITypes.OPENAI, config_type="system")
    reservation = FakeReservedLLMCall(provider_options={"openai": {"max_output_tokens": 4096}})
...
@pytest.mark.asyncio
async def test_send_once_marks_settlement_failed_when_usage_missing():
    llm_config = LLMConfig(model="gpt-4o", api_type=APITypes.OPENAI, config_type="system")
    reservation = FakeReservedLLMCall()
    billing = FakeBillingService(reservation=reservation)
    service = LLMExecutionService(llm_billing=billing)
    ...
    response = await service.send_once(
        ...
        messages=[message],
        billing_context=_billing_context(llm_config),
        usage_key="test_missing_usage",
    )

    assert response.finish_reason == FinishReason.END_TURN
    assert len(billing.settle_calls) == 0
    assert len(billing.release_calls) == 0
    assert len(billing.mark_failed_calls) == 1
    assert billing.mark_failed_calls[0].error == "missing_usage"


@pytest.mark.asyncio
async def test_send_once_raises_when_mark_settlement_failed_cannot_persist():
    llm_config = LLMConfig(model="gpt-4o", api_type=APITypes.OPENAI, config_type="system")
    reservation = FakeReservedLLMCall()
    billing = FakeBillingService(reservation=reservation)
    billing.mark_failed_error = RuntimeError("db write failed")
    service = LLMExecutionService(llm_billing=billing)
    client = FakeClient(
        [
            RunResponseOutput(
    ...
    with pytest.raises(BillingSettlementFinalError, match="persist settlement_failed"):
        await service.send_once(
            ...
            messages=[message],
            billing_context=_billing_context(llm_config),
            usage_key="test_mark_failed_raise",
        )
...
class FakeBillableTool:
    name = "search_tool"

    async def quote_cost(self, tool_call: ToolCallInput):
        assert tool_call.name == self.name
        return BillingQuote(
            strategy="bounded", reserve_usd=Decimal("0.03"), max_usd=Decimal("0.03")
        )


@pytest.mark.asyncio
async def test_tool_loop_bills_chat_tools_with_upfront_quotes(monkeypatch):
    llm_config = LLMConfig(model="claude-4-5-sonnet", config_type="system")
    llm_reservation = FakeReservedLLMCall()
    tool_reservation = SimpleNamespace(hold=SimpleNamespace(reservation_id="tool-res-1"))
    billing = FakeBillingService(
        reservation=llm_reservation,
        settle_result=SimpleNamespace(total_charged=Decimal("0.05")),
        tool_reservation=tool_reservation,
        tool_settle_result=SimpleNamespace(total_charged=Decimal("2.0")),
    )
    service = LLMExecutionService(llm_billing=billing, llm_invocation_repo=FakeInvocationRepo())
    client = FakeClient(
        [
            RunResponseOutput(
                content=[
    ...
    result = await service.run_tool_loop_until_final(
        ...
        tools=[],
        final_tool_name="final_tool",
        tool_registry={"search_tool": FakeBillableTool(), "final_tool": object()},
        max_loops=4,
        billing_context=_billing_context(llm_config),
        usage_key_prefix="test_tool_loop_billing",
    )

    assert result.final_payload == {"operations": [{"op": "set_text"}]}
    assert len(billing.reserve_tool_calls) == 1
    assert len(billing.settle_tool_calls) == 1
    assert len(billing.release_tool_calls) == 0
    assert billing.reserve_tool_calls[0]["tool_name"] == "search_tool"
    assert billing.settle_tool_calls[0]["actual_cost_usd"] == 0.03


@pytest.mark.asyncio
async def test_tool_loop_forwards_custom_tool_source_domain(monkeypatch):
    llm_config = LLMConfig(model="claude-4-5-sonnet", config_type="system")
    llm_reservation = FakeReservedLLMCall()
    tool_reservation = SimpleNamespace(hold=SimpleNamespace(reservation_id="tool-res-1"))
    billing = FakeBillingService(
    ...
    billing_context = LLMBillingContext(
        ...
        llm_config=llm_config,
        model_id=llm_config.model,
        tool_source_domain="storybook_tool",
    )

    await service.run_tool_loop_until_final(
        client=client,
        session_id="s1",
        messages=[user_message],
        tools=[],
        ...
        max_loops=4,
        billing_context=billing_context,
        usage_key_prefix="test_storybook_tool_loop",
    )

    assert billing.reserve_tool_calls[0]["source_domain"] == "storybook_tool"


class FakeZeroCostTool:
    name = "search_tool"
...
@pytest.mark.asyncio
async def test_tool_loop_records_zero_cost_usage_without_reservation(monkeypatch):
    llm_config = LLMConfig(model="claude-4-5-sonnet", config_type="system")
    billing = FakeBillingService(
        reservation=FakeReservedLLMCall(),
        settle_result=SimpleNamespace(total_charged=Decimal("0.05")),
    ...
    await service.run_tool_loop_until_final(
        ...
        tools=[],
        final_tool_name="final_tool",
        tool_registry={"search_tool": FakeZeroCostTool(), "final_tool": object()},
        max_loops=4,
        billing_context=_billing_context(llm_config),
        usage_key_prefix="test_zero_cost_tool_loop",
    )

    assert len(billing.reserve_tool_calls) == 0
    assert len(billing.zero_cost_tool_calls) == 1
    assert billing.zero_cost_tool_calls[0]["tool_name"] == "search_tool"
    assert billing.zero_cost_tool_calls[0]["succeeded"] is True
    assert billing.zero_cost_tool_calls[0]["source_domain"] == "chat_tool"


@pytest.mark.asyncio
async def test_tool_loop_records_zero_cost_usage_for_tool_errors(monkeypatch):
    llm_config = LLMConfig(model="claude-4-5-sonnet", config_type="system")
    billing = FakeBillingService(
        reservation=FakeReservedLLMCall(),
        settle_result=SimpleNamespace(total_charged=Decimal("0.05")),
    ...
    async def _fake_execute_tool(*, tool_call_id, tool_name, tool_input, tool_registry):
        return ToolResult(
            tool_call_id=tool_call_id,
            name=tool_name,
            output=ErrorTextContent(value="failed"),
            cost_usd=0.0,
        )
    ...
    await service.run_tool_loop_until_final(
        ...
        max_loops=4,
        billing_context=_billing_context(llm_config),
        usage_key_prefix="test_zero_cost_tool_error_loop",
    )

    assert len(billing.reserve_tool_calls) == 0
    assert len(billing.zero_cost_tool_calls) == 1
    assert billing.zero_cost_tool_calls[0]["succeeded"] is False
...
@pytest.mark.asyncio
async def test_send_once_writes_telemetry_without_using_caller_session():
    llm_config = LLMConfig(model="gpt-4o", api_type=APITypes.OPENAI, config_type="system")
    billing = FakeBillingService(
        reservation=FakeReservedLLMCall(),
        settle_result=SimpleNamespace(total_charged=Decimal("0.1")),
    ...
    invocation_repo = FakeInvocationRepo()
    service = LLMExecutionService(llm_billing=billing, llm_invocation_repo=invocation_repo)
    client = FakeClient(
        [
            RunResponseOutput(
                content=[TextContent(text="done")],
    ...
    response = await service.send_once(
        ...
        messages=[message],
        billing_context=billing_context,
        usage_key="test_separate_telemetry_session",
    )
