...
- IIAgent._validate_input / _initialize_session helpers
...
def _make_agent(model=None, **kwargs):
    ...
    with (
        patch("ii_agent.engine.v1.agents.agent.ServiceContainer.create", return_value=MagicMock()),
        patch("ii_agent.engine.v1.agents.sandbox_provider.SandboxProvider.__init__", return_value=None),
    ):
        ...
        agent._delegation_manager = MagicMock()
...
class TestIIAgentPublicAPI:
    ...
    def test_set_id_uses_name_when_id_is_none(self):
        ...
        agent.id = None
        with patch("ii_agent.engine.v1.agents.agent.generate_id_from_name", return_value="myagent-id"):
            agent.set_id()
    ...
    def test_add_sub_agent_appends(self):
        ...
        agent._delegation_manager.initialize_sub_agent.assert_called_once_with(sub)
    ...
    def test_as_tool_returns_base_agent_tool(self):
        ...
        agent = _make_agent()
        with patch("ii_agent.engine.v1.agents.agent.AgentAsTool") as mock_cls:
    ...
    async def test_cancel_run_delegates_to_global(self):
        ...
        with patch("ii_agent.engine.v1.agents.agent.cancel_run_global", mock_cancel):
            from ii_agent.engine.v1.agents.agent import IIAgent
            result = await IIAgent.cancel_run("run-123")
        assert result is True
        mock_cancel.assert_called_once_with("run-123")

    def test_acontinue_run_raises_when_no_run_id_and_no_run_response(self):
        agent = _make_agent()
    ...
    def test_acontinue_run_raises_when_both_run_id_and_run_response(self):
        ...
        with pytest.raises(ValueError, match="Only one"):
            agent.acontinue_run(run_id="some-run-id", run_response=rr)


# ---------------------------------------------------------------------------
# IIAgent._validate_input
# ---------------------------------------------------------------------------

class TestValidateInput:
    """Test the _validate_input helper method."""

    def test_string_passthrough(self):
        agent = _make_agent()
        result = agent._validate_input("hello world")
    ...
    def test_dict_passthrough(self):
        ...
        result = agent._validate_input(d)
        assert result == d

    def test_list_passthrough(self):
        agent = _make_agent()
        lst = ["hello", "world"]
        result = agent._validate_input(lst)
        assert result == lst
...
class TestIIAgentPostInit:
    ...
    def test_tools_becomes_empty_list_when_none(self):
        ...
        with (
            patch("ii_agent.engine.v1.agents.agent.ServiceContainer.create", return_value=MagicMock()),
            patch("ii_agent.engine.v1.agents.sandbox_provider.SandboxProvider.__init__", return_value=None),
            patch("ii_agent.engine.v1.agents.agent.NoOpSessionStore"),
        ):
            ...
            with (
                patch("ii_agent.engine.v1.agents.agent.MessageBuilder"),
                patch("ii_agent.engine.v1.agents.agent.ToolManager"),
                patch("ii_agent.engine.v1.agents.agent.ResponseHandler"),
                patch("ii_agent.engine.v1.agents.agent.HookExecutor"),
                patch("ii_agent.engine.v1.agents.agent.SandboxProvider"),
                patch("ii_agent.engine.v1.agents.agent.HITLHandler"),
                patch("ii_agent.engine.v1.agents.agent.DelegationManager"),
            ):
                agent.__post_init__()
...
class TestMessageBuilderGetUserMessage:
    ...
    def _make_builder(self, system_role="system"):
        from ii_agent.engine.v1.agents.message_builder import MessageBuilder
...
class TestDelegationManagerFindSubAgent:
    ...
    def _make_dm(self):
        from ii_agent.engine.v1.agents.delegation_manager import DelegationManager
...
class TestDelegationManagerInitializeSubAgent:
    ...
    def test_assigns_session_store_when_sub_agent_has_noop_store(self):
        from ii_agent.engine.v1.agents.delegation_manager import DelegationManager
        from ii_agent.engine.v1.agent_sessions.base import NoOpSessionStore
