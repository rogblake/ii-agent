"""Unit tests for ii_agent/engine/v1/run/agent.py.
...
class TestRunInput:
    ...
    def test_create_with_string_input(self):
        from ii_agent.engine.v1.run.agent import RunInput
...
class TestRunCompletedEvent:
    def test_default_event_field(self):
        from ii_agent.engine.v1.run.agent import RunCompletedEvent
    ...
    def test_status_can_be_set(self):
        from ii_agent.engine.v1.run.agent import RunCompletedEvent
        from ii_agent.engine.v1.run.base import RunStatus
...
class TestRunErrorEvent:
    def test_default_event_field(self):
        from ii_agent.engine.v1.run.agent import RunErrorEvent
...
class TestRunCancelledEvent:
    def test_default_event_field(self):
        from ii_agent.engine.v1.run.agent import RunCancelledEvent
...
class TestReasoningDeltaEvent:
    def test_default_event_field(self):
        from ii_agent.engine.v1.run.agent import ReasoningDeltaEvent
...
class TestRunEventTypeRegistry:
    ...
    def test_registry_contains_tool_call_completed(self):
        from ii_agent.engine.v1.run.agent import RUN_EVENT_TYPE_REGISTRY, ToolCallCompletedEvent
    ...
    def test_registry_contains_reasoning_started(self):
        from ii_agent.engine.v1.run.agent import RUN_EVENT_TYPE_REGISTRY, ReasoningStartedEvent
    ...
    def test_run_output_event_from_dict_raises_for_unknown_type(self):
        from ii_agent.engine.v1.run.agent import run_output_event_from_dict

        with pytest.raises(ValueError, match="Unknown event type"):
            run_output_event_from_dict({"event": "NonExistentEvent"})

    def test_run_output_event_from_dict_creates_run_started(self):
        from ii_agent.engine.v1.run.agent import run_output_event_from_dict, RunStartedEvent
