...
def make_mock_client(description="Test Agent", extensions=None) -> AsyncMock:
    ...
    client.call_agent = AsyncMock(return_value={
        "success": True,
        "content": "result text",
...
class TestExecuteDeep:
    ...
    async def test_execute_unknown_url_tries_to_get_client(self):
        ...
        mock_client = make_mock_client()
        mock_client.get_agent_card = AsyncMock(return_value=MagicMock(
            description="Dynamic", extensions=[]
        ))
...
class TestGetClientDeep:
    ...
    async def test_get_client_returns_cached_client_for_same_headers(self):
        ...
        with patch.object(tool, "_resolve_headers", return_value={}), \
             patch(
                 "ii_agent.agent.runtime.tools.a2a.a2a_agent_tool.IIAgentA2AClient",
             ) as MockClient:
...
class TestInitializeDeep:
    ...
    async def test_initialize_caches_agent_description_from_default_agents(self):
        tool = make_tool({"agent1": {"url": "http://agent1.example.com", "description": "My Agent"}})
