...
def _make_event(event_type: EventType, content: Any = None) -> RealtimeEvent:
    # RealtimeEvent.content is typed as dict[str, Any] in Pydantic, so we must
    # always construct with a valid dict, then override the value if needed.
    event = RealtimeEvent(
    ...
    if not isinstance(content, dict):
        # Bypass Pydantic validation for non-dict content (edge case tests)
...
class TestPublishAndAddEvent:
    ...
    async def test_add_event_enqueues_converted_events(self):
        ...
        event = _make_event(EventType.COMPLETE, {"text": "done"})
        await adapter.add_event(event)
...
class TestConvertEvent:
    ...
    def test_completion_events(self):
        ...
        adapter = _make_adapter()
        for event_type in [EventType.STREAM_COMPLETE, EventType.COMPLETE]:
    ...
    def test_sub_agent_complete_event(self):
        ...
        adapter = _make_adapter()
        event = _make_event(EventType.SUB_AGENT_COMPLETE, {"text": "sub done"})
        result = adapter._convert_event(event)
        assert len(result) == 1
        assert result[0].status.state == TaskState.working
    ...
    def test_interrupted_event(self):
        ...
        adapter = _make_adapter()
        event = _make_event(EventType.AGENT_RESPONSE_INTERRUPTED, {"text": "need input"})
        result = adapter._convert_event(event)
        assert len(result) == 1
...
class TestStatusBuilders:
    ...
    def test_status_input_required_not_final(self):
        adapter = _make_adapter()
        event = _make_event(EventType.AGENT_RESPONSE_INTERRUPTED, {"text": "waiting"})
    ...
    def test_status_sub_agent_not_final(self):
        adapter = _make_adapter()
        event = _make_event(EventType.SUB_AGENT_COMPLETE, {})
...
class TestArtifactUpdate:
    def test_empty_text_returns_empty_list(self):
        ...
        event = _make_event(EventType.AGENT_THINKING)
        event.content = None
    ...
    def test_text_content_produces_artifact_event(self):
        ...
        adapter = _make_adapter()
        event = _make_event(EventType.AGENT_RESPONSE, {"text": "Some response"})
        result = adapter._artifact_update(event)
        assert len(result) == 1
    ...
    def test_artifact_append_for_same_stream_key(self):
        ...
        event1 = _make_event(EventType.AGENT_RESPONSE, {"text": "chunk 1"})
        result1 = adapter._artifact_update(event1)
        assert result1[0].append is False

        # Second event appends
        event2 = _make_event(EventType.AGENT_RESPONSE, {"text": "chunk 2"})
    ...
    def test_runtime_trace_adds_sequence(self):
        ...
        event = _make_event(EventType.AGENT_RESPONSE, {"text": "traced"})
        result = adapter._artifact_update(event)
        assert len(result) == 1
    ...
    def test_runtime_trace_increments_sequence(self):
        adapter = _make_adapter(runtime_trace=True)
        event1 = _make_event(EventType.AGENT_RESPONSE, {"text": "a"})
        event2 = _make_event(EventType.AGENT_THINKING, {"text": "b"})
...
class TestArtifactText:
    def test_tool_call_returns_calling_description(self):
        adapter = _make_adapter()
        event = _make_event(
            EventType.TOOL_CALL, {"tool_name": "web_search", "tool_input": {"q": "hi"}}
        )
    ...
    def test_tool_result_non_message_tool_returns_none(self):
        adapter = _make_adapter()
        event = _make_event(
            EventType.TOOL_RESULT, {"tool_name": "web_search", "result": "results..."}
        )
        text = adapter._artifact_text(event)
...
class TestArtifactName:
    def test_returns_capitalized_event_type(self):
        adapter = _make_adapter()
        event = _make_event(EventType.AGENT_RESPONSE, {})
...
class TestResolveStreamKey:
    def test_streaming_event_with_tool_name(self):
        adapter = _make_adapter()
        event = _make_event(EventType.AGENT_RESPONSE, {"tool_name": "my_tool"})
    ...
    def test_streaming_event_returns_event_type_as_key(self):
        ...
        event = _make_event(EventType.AGENT_RESPONSE, {"text": "hello"})
        key = adapter._resolve_stream_key(event)
        assert key == EventType.AGENT_RESPONSE.value
    ...
    def test_streaming_event_with_stream_key_in_content(self):
        adapter = _make_adapter()
        event = _make_event(EventType.AGENT_RESPONSE, {"stream_key": "custom_key"})
        key = adapter._resolve_stream_key(event)
...
class TestBuildMessage:
    ...
    def test_build_status_event_with_text(self):
        ...
        event = adapter._build_status_event(
            TaskState.working, text="working on it", final=False
        )
    ...
    def test_build_status_event_without_text(self):
        ...
        adapter = _make_adapter()
        event = adapter._build_status_event(
            TaskState.completed, text=None, final=True
        )
