...
class TestLucideCatalogR4:
    ...
    def test_list_icons_with_query_filters(self):
        from ii_agent.projects.design.utils.lucide_catalog import list_icons, _list_available_icon_names
    ...
    def test_icon_node_to_svg_inner_escapes_attributes(self):
        ...
        assert "&lt;" in result or "&amp;" in result or ">" not in result.replace("<path", "").replace("/>", "")
...
class TestRuntimeInjectorR4:
    ...
    def test_sanitize_legacy_no_editable_style_blocks(self):
        from ii_agent.projects.design.utils.runtime_injector import sanitize_legacy_editable_artifacts
    ...
    def test_sanitize_legacy_removes_editable_style(self):
        from ii_agent.projects.design.utils.runtime_injector import sanitize_legacy_editable_artifacts
        html = (
            "<style>.editable { color: red; } #ff6b75 { display: none; }</style>"
            "<div>Content</div>"
        )
        result = sanitize_legacy_editable_artifacts(html)
    ...
    def test_sanitize_legacy_removes_editable_class(self):
        from ii_agent.projects.design.utils.runtime_injector import sanitize_legacy_editable_artifacts, EDITABLE_CLASS_NAMES
...
class TestIframeGetIconSvgToolR4:
    def _make_tool(self):
        from ii_agent.projects.design.tools.iframe_get_icon_svg import DesignModeIframeAIGetIconSvgTool
