...
class FakeSessionRepo:
    ...
    async def get_by_id_and_user(self, db, session_id, user_id):
        ...
        if s and s.user_id == user_id and s.deleted_at is None:
            return s
        return None
    ...
    async def get_sandbox_id(self, db, session_id):
        s = self.sessions.get(str(session_id))
        return s.sandbox_id if s else None
...
class FakeAgentRunService:
    ...
    async def get_running_by_session(self, db, session_id):
        return None
...
class TestGetPublicSessionDetails:
    ...
    @pytest.mark.asyncio
    async def test_returns_dict_for_public_session(self):
        svc = _make_service()
        sid = str(uuid.uuid4())
...
class TestSessionHasSandbox:
    @pytest.mark.asyncio
    async def test_returns_false_when_no_sandbox(self):
        svc = _make_service()
        sid = uuid.uuid4()
        session = _make_session_ns(id=str(sid), sandbox_id=None)
        svc._session_repo.sessions[str(sid)] = session
        result = await svc.session_has_sandbox(None, sid)
        assert result is False

    @pytest.mark.asyncio
    async def test_returns_true_when_sandbox_exists(self):
        svc = _make_service()
        sid = uuid.uuid4()
        session = _make_session_ns(id=str(sid), sandbox_id="sandbox-abc")
        svc._session_repo.sessions[str(sid)] = session
        result = await svc.session_has_sandbox(None, sid)
        assert result is True
...
class TestGetSessionUserId:
    ...
    async def test_returns_user_id(self):
        svc = _make_service()
        sid = uuid.uuid4()
        session = _make_session_ns(id=str(sid), user_id="u-99")
        svc._session_repo.sessions[str(sid)] = session
        result = await svc.get_session_user_id(None, sid)
        assert result == "u-99"

    @pytest.mark.asyncio
    async def test_returns_none_when_not_found(self):
        svc = _make_service()
        result = await svc.get_session_user_id(None, uuid.uuid4())
        assert result is None
...
class TestGetSessionsWithRunningStatus:
    ...
    async def test_returns_sessions_for_running_ids(self):
        ...
        session = _make_session_ns(id=sid)
        svc._session_repo.sessions[sid] = session
        svc._agent_run_service.running_session_ids = [sid]
        result = await svc.get_sessions_with_running_status(None)
        assert len(result) == 1
...
class TestGetOrCreateSession:
    ...
    async def test_returns_existing_session(self):
        ...
        with patch.object(svc, "find_session_by_id_info", return_value=mock_info):
...
class TestSessionToDict:
    def test_basic_dict_structure(self):
        ...
        with patch("ii_agent.sessions.service.sa_inspect") as mock_inspect:
            ...
            mock_state.unloaded = {"project"}
            mock_inspect.return_value = mock_state
            result = SessionService._session_to_dict(session)
    ...
    def test_skips_project_when_include_project_false(self):
        session = _make_session_ns()
        result = SessionService._session_to_dict(session, include_project=False)
