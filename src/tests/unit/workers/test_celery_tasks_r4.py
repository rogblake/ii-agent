...
# ---------------------------------------------------------------------------
# _check_user_credits (mocked DB)
# ---------------------------------------------------------------------------

class TestCheckUserCredits:
    @pytest.mark.asyncio
    async def test_returns_false_when_no_balance(self):
        from ii_agent.workers.celery.tasks import _check_user_credits

        mock_container = MagicMock()
        mock_container.credit_service.get_balance = AsyncMock(return_value=None)
        ...
        with (
            patch("ii_agent.workers.celery.tasks.get_celery_container", return_value=mock_container),
            patch("ii_agent.core.db.manager.get_db_session_local", return_value=mock_db_ctx),
        ):
            has_credits, available = await _check_user_credits("user-1", 100.0)
            assert has_credits is False
            assert available == 0.0

    @pytest.mark.asyncio
    async def test_returns_true_when_sufficient_credits(self):
        from ii_agent.workers.celery.tasks import _check_user_credits

        mock_balance = MagicMock()
        mock_balance.credits = 200.0
        mock_balance.bonus_credits = 50.0

        mock_container = MagicMock()
        mock_container.credit_service.get_balance = AsyncMock(return_value=mock_balance)
        ...
        with (
            ...
            assert available == 250.0

    @pytest.mark.asyncio
    async def test_returns_false_when_insufficient_credits(self):
        from ii_agent.workers.celery.tasks import _check_user_credits

        mock_balance = MagicMock()
        mock_balance.credits = 10.0
        mock_balance.bonus_credits = 5.0
...
# ---------------------------------------------------------------------------
# _deduct_storybook_credits (mocked DB)
# ---------------------------------------------------------------------------

class TestDeductStorybookCredits:
    @pytest.mark.asyncio
    async def test_returns_true_when_zero_credits(self):
        from ii_agent.workers.celery.tasks import _deduct_storybook_credits

        result = await _deduct_storybook_credits(
            user_id="u1",
            session_id="s1",
            credits_amount=0.0,
            description="test",
    ...
    @pytest.mark.asyncio
    async def test_returns_true_on_successful_deduction(self):
        from ii_agent.workers.celery.tasks import _deduct_storybook_credits

        mock_container = MagicMock()
        mock_container.credit_service.deduct_and_track_session_usage = AsyncMock(return_value=True)
    ...
    @pytest.mark.asyncio
    async def test_returns_false_on_failed_deduction(self):
        from ii_agent.workers.celery.tasks import _deduct_storybook_credits

        mock_container = MagicMock()
        mock_container.credit_service.deduct_and_track_session_usage = AsyncMock(return_value=False)
